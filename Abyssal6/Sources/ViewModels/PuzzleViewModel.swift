...
final class PuzzleViewModel {
    ...
    init() {
        ...
        for state in 0..<switchStates.count {
            switchStates[state] = Bool.random()
        }
        recalculateVoltage()
    }
    ...
    private func recalculateVoltage() {
        ...
        for state in 0..<switchStates.count where switchStates[state] {
            numerator += voltageSources[state] / resistors[state]
            denominator += 1.0 / resistors[state]
            anySwitchOn = true
        }
