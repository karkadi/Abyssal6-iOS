...
struct CharacterInteractionSheet: View {
    ...
    var body: some View {
        VStack(spacing: 20) {
            ...
            HStack(spacing: 20) {
                AbyssalButton(title: Lang.string("cancel"), color: .abyssalDanger, scaleY: 1.0) {
                    onComplete(nil)
                }
                if mode == .give && selectedItem != nil {
                    AbyssalButton(title: Lang.string("give_button"), color: .abyssalSuccess, scaleY: 1.0) {
                        onComplete(selectedItem)
                    }
                } else if mode == .talk {
                    AbyssalButton(title: Lang.string("talk_button"), color: .abyssalAccent, scaleY: 1.0) {
                        onComplete(nil)
                    }
                }
            }
