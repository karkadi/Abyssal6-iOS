...
struct TransitionOverlayView: View {
        ...
        GeometryReader { geometry in
            ZStack {
                ...
                GIFImage(source: fullURL)
                    ...
                    .animation(.easeInOut(duration: viewModel.transitioningToNewImage ? 0.5 : 0.0), value: viewModel.backgroundImageName)
    ...
    private func itemImageView(for overlay: ItemOverlay) -> some View {
        ...
        let glowColor = Color.yellow.opacity(0.7)

        Image(overlay.imageName)
            .resizable()
            .scaledToFit()
            .frame(width: size, height: size)
            .shadow(color: glowColor, radius: 8 * scaleFactor, x: 0, y: 0)  // outer glow
            .overlay(
                RoundedRectangle(cornerRadius: 8 * scaleFactor)
                    .stroke(
                        LinearGradient(
                            colors: [.white, glowColor, .white],
                            startPoint: .topLeading,
                            endPoint: .bottomTrailing
                        ),
                        lineWidth: 2 * scaleFactor
                    )
                    .blur(radius: 1 * scaleFactor)
            )
